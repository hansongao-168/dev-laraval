...
> 本文档为 `gz168/Wechat` 模块家族的全量设计与落地记录，覆盖微信生态全部本期目标能力，替代早期只含"账户 + 登录 + 支付骨架"的范围稿。**五个阶段已全部实施完成（2026-08-30），各阶段验收状态见 §4**；剩余为依赖真实凭据/商户资质的人工联调验收项。
...
7. **已知限制**：支付响应验签为尽力严格模式（响应携带签名头且平台证书可得时强制验证；证书轮换期未知序列号降级告警跳过——回调仍是资金状态唯一信任来源）；`oa_mass` 群发走 Admin 直调 API 而非通知编排渠道（多接收者不适合单 openid 渠道接口）——均为有意取舍，如需统一再演进。
